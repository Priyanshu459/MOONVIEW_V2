...
function updateUserInHeader(user) {
    ...
    if (user?.localUser) {
        ...
        // Revert phase 3 Moonview custom hiding, let the shell manager handle it
        const policy = user.Policy ? user.Policy : user.localUser.Policy;
        const isAdmin = policy?.IsAdministrator;
        
        document.body.classList.remove('hideMainDrawer');
        if (mainDrawerButton) mainDrawerButton.classList.remove('hide');
...
const fetchServerName = (_apiClient) => {
    _apiClient
        ?.getPublicSystemInfo()
        .then(({ ServerName }) => {
            documentTitle = ServerName === 'Jellyfin' ? 'Moonview' : (ServerName || documentTitle);
